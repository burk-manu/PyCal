...
class AdvancedCalculator:
    def __init__(self, root):
        ...
        self.entry = tk.Entry(root, font=("Arial", 24), justify="right", bd=10, relief=tk.RIDGE)
        self.entry.pack(fill=tk.BOTH, ipadx=8, pady=10, padx=10)

        # History display
        self.history_label = tk.Label(root, text="", font=("Arial", 12), bg="#f0f0f0", fg="#555")
        ...
        for i, row in enumerate(buttons):
            for j, text in enumerate(row):
                button = tk.Button(
                    button_frame, text=text, font=("Arial", 18), padx=20, pady=20,
                    bg="#e0e0e0", fg="#000", bd=2, relief=tk.RAISED,
